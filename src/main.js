...
function buildCity(){
  ...
  CONFIG.buildings.forEach((cfg,i)=>{
    ...
    const b={ cfg, index:i, group, cityObj, detailObj,
...
function loadModelFiles(files){
  ...
  return entry.arrayBuffer().then(async buf=>{
    ...
    made.push(rootUrl);
    gl.load(rootUrl,
      g=>{ made.forEach(u=>URL.revokeObjectURL(u));
...
function camStateFor(fr){
  return { pos:fr.target.clone().add(dirFrom(fr.az,fr.el).multiplyScalar(CAM_DIST)),
...
addEventListener('keydown',e=>{
  ...
  if((e.key==='e'||e.key==='E') && !/^(INPUT|TEXTAREA|SELECT)$/.test(document.activeElement.tagName)) editor.toggle();
});
...
$('edToggle').addEventListener('click',()=>editor.toggle());
...
/* Text and colours */
const textMap={tTitle:'title',tSubtitle:'subtitle',tHintCity:'hintCity',tHintBuilding:'hintBuilding',
...
loader.load(modelUrl,gltf=>{
  ...
  if(new URLSearchParams(location.search).has('edit')) editor.open();
  requestAnimationFrame(frame);
},
xhr=>{
